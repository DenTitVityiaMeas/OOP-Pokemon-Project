...
void BattleChariVsGren(string trainerName) {
  ...
  // Make buttons
  Button m1(Charizard.M1.getMoveName(), {120, 40}, 16, sf::Color::Red,
            sf::Color::Black);
  m1.setFont(courierNew);
  m1.setPosition({435, 420});
  Button m2(Charizard.M2.getMoveName(), {120, 40}, 16, sf::Color::Red,
            sf::Color::Black);
  m2.setFont(courierNew);
  m2.setPosition({565, 420});
  Button m3(Charizard.M3.getMoveName(), {120, 40}, 16, sf::Color::Red,
            sf::Color::Black);
  m3.setFont(courierNew);
  m3.setPosition({435, 470});
  Button m4(Charizard.M4.getMoveName(), {120, 40}, 16, sf::Color::Red,
            sf::Color::Black);
  ...
  while (window.isOpen()) {
      ...
      switch (event.type) {
        ...
        case sf::Event::MouseButtonPressed:
          if (m1.isMouseOver(window)) {
            if (skillAvailable == 1) {
              battleF(skillAvailable, round, Charizard, Geninja, 1,
                      trainerName);
            ...
            if (skillAvailable == 1) {
              battleF(skillAvailable, round, Charizard, Geninja, 2,
                      trainerName);
            } else if (skillAvailable == 0) {
              battleF(skillAvailable, round, Charizard, Geninja, 2,
                      trainerName);
            ...
            if (skillAvailable == 1) {
              battleF(skillAvailable, round, Charizard, Geninja, 3,
                      trainerName);
              round++;
              skillAvailable = 0;
            }
            ...
            if (skillAvailable == 1) {
              battleF(skillAvailable, round, Charizard, Geninja, 4,
                      trainerName);
            } else if (skillAvailable == 0) {
              battleF(skillAvailable, round, Charizard, Geninja, 4,
                      trainerName);
    ...
    if (Charizard.getHealth() < (10 * 0.5)) {
      hpBarA.setFillColor(sf::Color::Yellow);
    }
    if (Charizard.getHealth() < (10 * 0.25)) {
    ...
    hpBarA.setSize(sf::Vector2f(Charizard.getHealth() * 21.5, 20));
